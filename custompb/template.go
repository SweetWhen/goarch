...
func (s *service) execute() string {
	tmpMethodSet := make(map[string]*method, len(s.Methods))
	tmpMethodKeys := make([]string, 0, len(s.Methods))
	for _, m := range s.Methods {
		m := m
		tmpMethodSet[m.Name] = m
		tmpMethodKeys = append(tmpMethodKeys, m.Name)
	}
	sort.Strings(tmpMethodKeys)
	s.DistMethods = make([]*method, 0, len(tmpMethodSet))
	for _, k := range tmpMethodKeys {
		s.DistMethods = append(s.DistMethods, tmpMethodSet[k])
	}
